...
class Application(Frame):
	def __init__(self, master=None):
		...
		button2 = Button(master, text="", command=self.create_window_Rules).grid(row=7,column=1,sticky=E+W) #Injection Rules
	...
	def create_window_Rules(self):
		global t
		t = Toplevel(root)
		t.geometry("300x50+200+200")
		t.wm_title("Virtual CAN Interface")
		global e
		e = Entry(t)
		e.pack()
		e.focus_set()
		b = Button(t,text='Confirm',command=self.storeInterface)
		b.pack(side='bottom')

	def storeInterface(self):
		CANinterface = e.get()
		interface_stored = 1 
		t.destroy()
		stop = True
	...
	def Cap(self):
		...
		db = MySQLdb.connect("localhost","root","toor","TRAFFIC" )
